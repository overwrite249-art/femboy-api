...
test("MongoDB replica-set accounting invariants", { skip: !uri }, async (t) => {
	assert.match(
		uri!,
		/^mongodb:\/\/(?:127\.0\.0\.1|localhost)(?::\d+)?\//,
		"integration tests are loopback-only",
	)
	...
	try {
		await t.test(
			"concurrent reservations cannot oversell durable user and token balances",
			async () => {
				const who = await account("race")
				const outcomes = await Promise.allSettled(
					Array.from({ length: 10 }, (_, i) =>
						reserveQuota(who, `race-${i}`, 200),
					),
				)
				assert.equal(
		...
		await t.test(
			"rollback restores every document touched by a failed transaction",
			async () => {
				...
				await assert.rejects(
					db.transaction(async (tx) => {
						await tx
							.collection("users")
							.updateOne({ _id: who.userId }, { $inc: { quota: -100 } })
						await tx
							.collection("tokens")
							.updateOne({ _id: who.tokenId }, { $inc: { quota: -100 } })
		...
		await t.test(
			"duplicate usage deliveries cannot increment derived rollups again",
			async () => {
				const input = {
				...
				await Promise.all(
					Array.from({ length: 6 }, () =>
						recordUsage(input, { buffered: false }),
					),
				)
				const row = await (
					await usageRollups()
				).findOne({ _id: `user:usage-user:${hourBucket()}` })
				assert.equal(row?.requests, 1)
				assert.equal(row?.quota, 50)
			},
		)
		await t.test(
			"a measured settlement survives a temporary Mongo outage and replays once",
			async () => {
				...
				try {
					assert.equal(
						(await finalizeQuota(who, "recovery", 125)).applied,
						false,
					)
				} finally {
		...
		await t.test(
			"console balance compare-and-set rejects stale edits on MongoDB",
			async () => {
				const who = await account("console-balance")
				const outcomes = await Promise.allSettled([
					updateUser(who.userId, { quota: 900, expectedQuota: 1000 }),
					updateUser(who.userId, { quota: 800, expectedQuota: 1000 }),
				])
				assert.equal(
					outcomes.filter((result) => result.status === "fulfilled").length,
					1,
				...
				await assert.rejects(
					updateUser(who.userId, { quota: 2000, expectedQuota: 1000 }),
					/balance changed/,
				)
				assert.ok([800, 900].includes(await currentBalance(who)))
			},
		)
		await t.test(
			"multiple accounts may omit email, but usernames remain unique",
			async () => {
				await createUser({ username: "no-email-a" })
				await createUser({ username: "no-email-b" })
				const results = await Promise.allSettled([
					createUser({ username: "duplicate-name" }),
					createUser({ username: "duplicate-name" }),
				])
